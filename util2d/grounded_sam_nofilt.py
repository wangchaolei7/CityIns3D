...
# from util2d.mask_filtering import mask_filter, recover_filted_mask # from MaskClustering 
#### Open3DIS util
...
class Grounded_Sam:
    ...
    def __init__(self, cfg):
        ...
        self.siglip_model = AutoModel.from_pretrained("google/siglip-so400m-patch14-384").to(device)
    ...
    def gen_grounded_mask_and_feat(self, scene_id, class_names, cfg, gen_feat=True):
        ...
        # Accmulate CLIP mask feature onto 3D point cloud ?
        if gen_feat:
            grounded_features = torch.zeros((n_points, cfg.foundation_model.siglip_dim), dtype=torch.float16).cuda()
        else:
            grounded_features = None

        # 保存不同box_threshold的mask数量
        # save_path = f"/home/wcl/exp_scannet200/version_Grounded-SAM/vis_mask/box_counts_{cfg.foundation_model.box_threshold}.txt"
        ### ScanNet200
        # vismask_dir = f"/home/wcl/Open3DIS/data/Scannet200/Scannet200_2D_5interval/val/{scene_id}/output/mask"
        ### ScanNetpp
        # vismask_dir = f"/home/wcl/Open3DIS/data/Scannetpp/Scannetpp_2D_5interval/val/{scene_id}/output/mask"
        # os.makedirs(vismask_dir, exist_ok=True)
        # mask_box_dir = f'{cfg.data.datapath}/{scene_id}/output/mask_box'
        # os.makedirs(mask_box_dir, exist_ok=True)


        vismask_dir = f"/data3/wcl/Open3DIS/Matterport3d/scans/{scene_id}/output/mask_nofilt"
        # vismask_dir = f"/data3/wcl/Open3DIS/Replica/replica_2d/{scene_id}/output/mask_nofilt"
        os.makedirs(vismask_dir, exist_ok=True)
        ...
        for i in trange(0, len(loader), cfg.data.img_interval):
            ...
            # frame_id = frame["depth_frame_id"]  # str
            ...
            segment_size = 10 # scannetpp_benchmark
            ...
            ) # use box-prompt to segment         
            if masks == None:  # ⭐No mask in the view
                continue
            
            vis_image = self.visualize_masks(image_pil, masks)
            output_path = os.path.join(vismask_dir, f"{frame_id}.png")
            ...
            for box_id, box in enumerate(boxes_filt):
                ...
                regions.append(self.siglip_processor(images=Image.fromarray((tmp.cpu().numpy())), return_tensors="pt"))

            masks = torch.logical_and(masks, masks_fitted)  # fitting
            tensors = [region['pixel_values'] for region in regions]
            imgs = torch.stack(tensors).cuda()
            ...
            with torch.no_grad(), torch.cuda.amp.autocast(): 
                for img_batch in img_batches:
                    ...
                    img_batch = {'pixel_values': img_batch.float()}
                    image_feat = self.siglip_model.get_image_features(**img_batch)
            ...
            if gen_feat:
                ...
                elif "matterport3d" in cfg.data.dataset_name:
                    mapping = torch.ones([n_points, 4], dtype=int, device='cuda')
                    mapping[:, 1:4] = pointcloud_mapper.compute_mapping_torch(pose, points, depth, intrinsic)
                ...
                final_feat = torch.einsum("qc,qhw->chw", image_features, pred_masks.tensor.half())
                ### Summing features
    ...
    def visualize_masks(self, image, masks, colors=None):
        # 确保masks是numpy数组
        ...
        # 如果没有提供颜色，则为每个mask生成随机颜色
        if colors is None:
            num_masks = masks.shape[0]
            colors = [tuple(np.random.randint(0, 256, 3)) for _ in range(num_masks)]
        elif not isinstance(colors, list) or len(colors) != masks.shape[0]:
            raise ValueError("The provided colors list must match the number of masks.")

        # 创建一个与原图大小相同的彩色掩膜
        mask_image = np.zeros_like(image)

        # 对于每一个mask，在mask_image上绘制
        for i, mask in enumerate(masks):
            # 移除大小为1的维度
            mask = np.squeeze(mask).astype(np.bool)

            # 在mask_image上应用当前mask
            mask_image[mask] = colors[i]

        # 将mask_image和原始图像混合
        alpha = 0.8  # 调整这个值可以改变mask的透明度
